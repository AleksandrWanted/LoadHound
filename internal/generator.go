...
func GetGenerators(args string) ([]GeneratorFunc, error) {
	...
	for _, arg := range argsSplit {
		...
		case "randStringName":
			if len(funcSignature) > 1 {
				return nil, fmt.Errorf("invalid function signature, randStringName() does not support args: %v", funcSignature)
			}
			generators = append(generators, func() any { return RandStringName() })

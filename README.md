...
```bash
# Run a load test
loadhound -run test-example.toml
```
...
```bash
go install github.com/AleksandrWanted/LoadHound@latest
```

### From Source

```bash
git clone https://github.com/AleksandrWanted/LoadHound.git
cd loadhound
...
| `randStringName` | Random fake name | `string` |
